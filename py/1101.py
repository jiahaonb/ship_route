...
# 数据输出地址
file = open("1031.txt", "w", encoding="utf-8")
...
# 距离参数
l_p = {1:100.89, 2:448.95, 3:186.94,4:200.12,5:20.75}  # 从港口 p 到 p+1 的不绕行距离（海里）
...
for n in range(max_iter):
    ...
    for omega in Omega:
        ...
        if model.status == GRB.OPTIMAL:
            ...
            print(f"燃油消耗：")
            for p in P:
                print(f"{scenario_solutions[omega]['R_H'][p]:.12f}")
                print(f"港口{p}的绕行距离是{scenario_solutions[omega]['l_ne'][p]}")
            for p in P:
                print(f"港口 {p}:")
                # 加油策略
                ...
                # 绕行策略（仅对 p in P_）
                if p in P_:
                    detour_strategy = '绕行' if scenario_solutions[omega]['y'][p] == 1 else '直接航线'
