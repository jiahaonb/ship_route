...
# 数据输出地址
file = open("最终z2.txt", "w", encoding="utf-8")
...
for n in range(max_iter):
    ...
    # 对于每个情景，建立并求解子问题
    # 每个迭代，都需要初始化我们的函数值
    xs = {omega: float(0) for omega in Omega}
    scenario_objs = []
    for omega in Omega:
        # 创建 Gurobi 模型 参数如下：
        ...
        penalty_xs = quicksum(
            rho_xs / 2 * (xs[omega] - xs_bar) ** 2 + mu_z[omega][p, k] * (z[p, k] - z_bar[p, k]) for p in P for k in K
        )
        ...
        if n == 0:
            ...
            model.setObjective(
                FirstStageCost + SecondStageCost + penalty_xs,
                GRB.MINIMIZE
            )
        ...
        if model.status == GRB.OPTIMAL:
            ...
            scenario_solutions[omega] = {
                ...
                'obj': model.ObjVal, # 这个就是set_Objetive的解，就是最小的成本
                # 存储二阶段变量以便输出
            ...
            # 保存我们的最小函数值obj
            xs[omega] = scenario_solutions[omega]['obj']
