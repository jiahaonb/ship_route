...
for n in range(max_iter):
    ...
    for omega in Omega:
        ...
        for p in P:
            ...
            # z[p,k] 与 x[p,k] 的关系
            for k in K:
                model.addConstr(z[p, k] >= x[p, k] - (1 - z[p, k]), name=f"Z_X_Relation1_{p}_{k}")
                model.addConstr(z[p, k] <= x[p, k] + (1 - z[p, k]), name=f"Z_X_Relation2_{p}_{k}")
            # 只能选择一个时间窗口
        ...
        for p in P:
            if p == 1:
                ...
                for k in K:
                    model.addConstr(
                        t_port_entry_mod[p] >= a[p, k] - M_big * (1 - z[p, k]),
                        name=f"PortEntryStartWindow_{p}_{k}"
                    )
                    model.addConstr(
                        t_port_entry_mod[p] <= b[p, k] + M_big * (1 - z[p, k]),
