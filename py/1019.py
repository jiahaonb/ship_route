...
for n in range(max_iter):
    ...
    for omega in Omega:
        ...
        penalty_v_eca = quicksum(
            rho_v_eca / 2 * (v_eca[p] - v_eca_bar[p])**2 + mu_v_eca[omega][p] * (v_eca[p] - v_eca_bar[p]) for p in P_
        )

        penalty_v_neca = quicksum(
            rho_v_neca / 2 * (v_neca[p] - v_neca_bar[p])**2 + mu_v_neca[omega][p] * (v_neca[p] - v_neca_bar[p]) for p in P_
        )

        penalty_y = quicksum(
            rho_y / 2 * (y[p] - y_bar[p])**2 + mu_y[omega][p] * (y[p] - y_bar[p]) for p in P_
        )
        ...
        # 求解模型
        # print(f"开始求解情景 {omega} 的模型...")
        model.optimize()
        # print(f"情景 {omega} 的模型求解完成。")
        ...
        if model.status == GRB.OPTIMAL:
            ...
            for p in P:
                ...
                # 最优速度
                if p in P_:
                    print(f"  ECA航速: {scenario_solutions[omega]['v_eca'][p]:.2f} 节")
                    print(f"  非ECA航速: {scenario_solutions[omega]['v_neca'][p]:.2f} 节")
                else:
                    print(f"  ECA航速: N/A")
                    print(f"  非ECA航速: N/A")
                # 加油量
                ...
                # 时间窗选择
                selected_window = max(scenario_solutions[omega]['x'][p, k] for k in K)
                for k in K:
                    print(f"时间窗口是：{selected_window}, 第k个时间窗的窗口是：{scenario_solutions[omega]['x'][p, k]}")
                    if scenario_solutions[omega]['x'][p, k] == selected_window:
                        print(f"  选择的时间窗口: {k}")
                # 到达时间是当日具体时间
                print(f"  当日到达时间（0-24h）: {scenario_solutions[omega]['t_eta_mod'][p]:.2f} 小时")
                print(f"  船的等待时间：{scenario_solutions[omega]['t_wait'][p]:.2f}")
                print(f"  船的迟到时间：{scenario_solutions[omega]['t_late'][p]:.2f}")
        else:
